...
final class CompressedTimeAxisTests: XCTestCase {
    ...
    private let threshold: TimeInterval = 45 * 60
    ...
    func testXPositionsIncreaseMonotonically() {
        let sut = axis([0, 300, 8 * 3600, 8 * 3600 + 300, 20 * 3600])
        var previous: CGFloat = -1
        for offset in stride(from: 0.0, through: 20 * 3600, by: 600) {
    ...
    func testSingleGapCollapsesToBand() {
        let sut = axis([0, 300, 8 * 3600, 8 * 3600 + 300])
        ...
        XCTAssertEqual(band.duration, 8 * 3600 - 300, accuracy: 0.01)
    ...
    func testLongSpanGetsMoreWidthThanShortSpan() {
        ...
        let secondSpan = stride(from: 12 * 3600.0, through: 12 * 3600.0 + 1200, by: 300.0).map { $0 }
    ...
    func testManyGapsStayWithinWidthBudget() {
        ...
        for index in 0..<31 {
            offsets.append(Double(index) * 24 * 3600)
            offsets.append(Double(index) * 24 * 3600 + 300)
        }
        let sut = axis(offsets)
    ...
    func testEdgeGapsAreTrimmed() {
        ...
        XCTAssertEqual(sut.x(for: base.addingTimeInterval(-24 * 3600)), 0, accuracy: 0.01)
        XCTAssertEqual(sut.x(for: base.addingTimeInterval(24 * 3600)), width, accuracy: 0.01)
    ...
    func testDateAtXRoundTripsWithinASecond() {
        let sut = axis([0, 1800, 3600, 12 * 3600, 12 * 3600 + 1800])

        let offsets: [TimeInterval] = [0, 900, 1800, 3600, 12 * 3600, 12 * 3600 + 900]
